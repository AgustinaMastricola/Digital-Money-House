...
export const GET = async (req: NextRequest, {params} : {params: Params}) => {
  ...
  try{
    const transferencesList = await fetch(`https://digitalmoney.digitalhouse.com/api/accounts/${id}/transferences`, {
      method: 'GET',
      headers: {
...
export const POST = async (req: NextRequest, {params} : {params: Params}) => {
  ...
  try{
  const results = await fetch(`https://digitalmoney.digitalhouse.com/api/accounts/${id}/transferences`,{ 
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization : token
